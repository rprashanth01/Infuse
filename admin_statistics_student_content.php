<?php
	...
	$stu_max_num_dept = 0;
	$query = $admin_statistics_student_dept_query;
	$result = mysql_query($query);
	while ($row = mysql_fetch_assoc($result)) {
		$dept[$row['STU_DEPT_ID']] = $row['NO_OF_STU'];
		if ($stu_max_num_dept < $row['NO_OF_STU']) {
			$stu_max_num_dept = $row['NO_OF_STU'];
		}
	}
	$stu_max_num_dept = round_off($stu_max_num_dept) + (int) round_off((int) ($stu_max_num_dept / 10));
	
	$stu_max_num_sem = 0;
	$sem[3] = $sem[2] = $sem[1] = $sem[0] = 0;
	$query = $admin_statistics_student_sem_query;
	$result = mysql_query($query);
	while ($row = mysql_fetch_assoc($result)) {
		switch ($row['STU_SEM']) {
			case 1:
	...
	$query = $admin_statistics_student_gender_query;
	$result = mysql_query($query);
	while ($row = mysql_fetch_assoc($result)) {
...
<div class="panel-group" id="admin_statistics_student_panel">
	
	<div class="panel panel-default">
		
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#admin_statistics_student_panel" href="#statistics_student_department">Student Statistics by Department</a>
			</h4>
		</div>
		
		<div id="statistics_student_department" class="panel-collapse collapse">
			<div class="panel-body text-center">
				<p>Number of students present in each department</p>
				<canvas id="student_dept" width="600" height="400"></canvas>
			</div>
		</div>
	...
	<div class="panel panel-default">
		...
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#admin_statistics_student_panel" href="#statistics_student_year">Student Statistics by Year</a>
			</h4>
		</div>
	...
	<div class="panel panel-default">
		...
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#admin_statistics_student_panel" href="#statistics_student_gender">Student Statistics by Gender</a>
			</h4>
		</div>
...
<script>
	...
	var dept = document.getElementById("student_dept").getContext('2d');
	new Chart(dept).Bar(deptData, {
		scaleOverride : true, 
		scaleSteps : 10,
		scaleStepWidth : <?php echo $stu_max_num_dept / 10; ?>,
		scaleStartValue : 0
	});
